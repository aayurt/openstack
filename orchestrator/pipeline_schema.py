...
NODE_PORT_MAP: dict[str, dict[str, list[str]]] = {
    ...
    NodeType.WORKER_ROUTER.value: {"inputs": ["input"], "outputs": ["worker_01", "worker_02", "worker_03"]},
    NodeType.OPENCODE_WORKER.value: {"inputs": ["input"], "outputs": ["output"]},
    NodeType.TEST.value: {"inputs": ["from_01", "from_02", "from_03"], "outputs": ["pass", "fail"]},
...
def default_pipeline() -> PipelineDefinition:
    return PipelineDefinition(
        ...
        version=1,
        description="Default coding task pipeline with 3 executors: classify → route → execute (3 workers) → test → triage",
        nodes=[
            PipelineNode(id="input", type=NodeType.TASK_INPUT, position={"x": 50, "y": 250}, label="Task Input"),
            PipelineNode(id="classify", type=NodeType.LAYA_CLASSIFY, position={"x": 250, "y": 250}, label="Laya Classify"),
            PipelineNode(id="readiness", type=NodeType.TASK_READINESS, position={"x": 450, "y": 250}, label="Readiness"),
            PipelineNode(id="router", type=NodeType.WORKER_ROUTER, position={"x": 650, "y": 250}, label="Worker Router"),
            PipelineNode(id="executor_01", type=NodeType.OPENCODE_WORKER, position={"x": 900, "y": 100}, label="Executor 01", config={"slot": "01"}),
            PipelineNode(id="executor_02", type=NodeType.OPENCODE_WORKER, position={"x": 900, "y": 250}, label="Executor 02", config={"slot": "02"}),
            PipelineNode(id="executor_03", type=NodeType.OPENCODE_WORKER, position={"x": 900, "y": 400}, label="Executor 03", config={"slot": "03"}),
            PipelineNode(id="tests", type=NodeType.TEST, position={"x": 1150, "y": 250}, label="Tests"),
            PipelineNode(id="triage", type=NodeType.LAYA_TRIAGE, position={"x": 1350, "y": 250}, label="Laya Triage"),
            PipelineNode(id="done", type=NodeType.DONE, position={"x": 1550, "y": 150}, label="Done"),
            PipelineNode(id="retry_backoff", type=NodeType.RETRY, position={"x": 1550, "y": 300}, label="Retry"),
            PipelineNode(id="hermes", type=NodeType.HERMES_REPLAN, position={"x": 1550, "y": 450}, label="Hermes Re-plan"),
        ],
        edges=[
            ...
            PipelineEdge(source="router", target="executor_01", source_port="worker_01"),
            PipelineEdge(source="router", target="executor_02", source_port="worker_02"),
            PipelineEdge(source="router", target="executor_03", source_port="worker_03"),
            PipelineEdge(source="executor_01", target="tests", target_port="from_01"),
            PipelineEdge(source="executor_02", target="tests", target_port="from_02"),
            PipelineEdge(source="executor_03", target="tests", target_port="from_03"),
            ...
            PipelineEdge(source="retry_backoff", target="router"),
        ],
    )
